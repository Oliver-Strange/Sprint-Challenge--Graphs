...
# def to keep track of visited rooms
def visitedRoom(graph, startRoom):
    # check directions of current room
    for direction in graph[startRoom]:
        # if they are still "?"
        if graph[startRoom][direction] == "?":
            # return none because they have not been visited
            return None

    # start a bfs for rooms to visit
    q = []
    ...
    while len(q) > 0:
        # start path with dequeue
        path = q.pop(0)
        # set the room as the path
        room = path[-1]
        # if the room is not in visited
        if room not in visited:
            # add it
            visited.add(room)

            # for the rest of the directions in the current room
            for direction in graph[room]:
                # if there are "?"
                if graph[room][direction] == "?":
                    # return those directions 
                    return path 

            # for the directions in the current room 
            for direction in graph[room]:
                # make a copy of the path
                newPath = path.copy()
                # establish current new room data 
                roomPath = graph[room][direction]
                # add the new room path 
                newPath.append(roomPath)
                # enqueue the new path 
                q.append(newPath)

    return None

def wander(graph, currentRoom):
    # if the current room id is not in the graph
    if currentRoom.id not in graph:
        # add the room with ? for directions available exits
        graph[currentRoom.id] = {i: "?" for i in currentRoom.getExits()}
        # add room to visitedRooms 
        visitedRooms.add(currentRoom.id)
        

# set up while loop for length of graph
while len(roomGraph5) != len(visitedGraph):
    # set this room as the player currentRoom
    thisRoom = player.currentRoom.id
    # recursively call wander 
    wander(visitedGraph, player.currentRoom)
    # for the directions in this room
    for direction in visitedGraph[thisRoom]:
        # if there are "?"
        if visitedGraph[thisRoom][direction] == "?":
            # add that direction to traversalPath
            traversalPath.append(direction)
            # go that direction
            player.travel(direction)
            # recursively call wander to set current room and check directions
            wander(visitedGraph, player.currentRoom)
            # set the new room as the players current room 
            newRoom = player.currentRoom.id
            # set the direction moved 
            newDirection = movingDirections[direction]
            # update the visitedGraph previous room to the new directions of new room
            visitedGraph[thisRoom][direction] = newRoom
            # update the visitedGraph with the current room and the direcions 
            visitedGraph[newRoom][newDirection] = thisRoom 
            # update previous room to new room 
            thisRoom = newRoom
            # stop the check
            break
    # set a way to go back through visited rooms
    goBack = visitedRoom(visitedGraph, player.currentRoom.id)
    # if there is a room to go back to
    if goBack is not None:
        # check the new rooms id
        for id in goBack:
            # for the directions in the going back room 
            for direction in visitedGraph[thisRoom]:
                # if the room id is already in graph
                if visitedGraph[thisRoom][direction] == id:
                    # add the direction to the traversalPath
                    traversalPath.append(direction)
                    # travel back that way
                    player.travel(direction)
...
if len(visited_rooms) == len(roomGraph5):
    print(f"TESTS PASSED: {len(traversalPath)} moves, {len(visited_rooms)} rooms visited")
else:
    print("TESTS FAILED: INCOMPLETE TRAVERSAL")
    print(f"{len(roomGraph5) - len(visited_rooms)} unvisited rooms")
